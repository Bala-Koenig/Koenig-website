...
const RELEVANT_TECHNOLOGIES = [
  { label: "Power Platform", prefixes: ["PL-"] },
  { label: "Azure",          prefixes: ["AZ-"] },
  { label: "Data & AI",      prefixes: ["DP-", "AI-"] },
  { label: "Microsoft 365",  prefixes: ["MS-", "MB-910"] },
  { label: "Security",       prefixes: ["SC-", "SY0-", "CS0-", "PT0-", "CAS-", "SCS-"] },
  { label: "Dynamics 365",   prefixes: ["MB-"] },
  { label: "Networking",     prefixes: ["200-301", "350-", "N10-", "CCIE-EI"] },
  { label: "Cloud",          prefixes: ["CLF-", "SAA-", "SAP-", "DVA-", "SOA-", "DEA-", "MLS-", "DOP-"] },
  { label: "DevOps",         prefixes: ["AZ-400", "DOP-"] },
  { label: "Virtualisation", prefixes: ["VCP-"] },
];
...
const MODE_BTNS = [
  {
    key: "courses" as const, label: "Courses", sub: "Pricing & Enroll",
    icon: (<><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></>),
  },
  {
    key: "exams" as const, label: "Exam Guide", sub: "Details & Skills",
    icon: (<><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M8 7h8M8 11h8M8 15h5"/><circle cx="17" cy="17" r="3"/><path d="m19.5 19.5 1.5 1.5"/></>),
  },
];
...
export default function CourseListings() {
  ...
  const [reqTab, setReqTab]                 = useState<"individual" | "enterprise">("individual");
  ...
  const [activeSubTechs, setActiveSubTechs] = useState(_emptySet);
  ...
  const toggleSubTech = (label: string) =>
    setActiveSubTechs(prev => { const s = new Set(prev); s.has(label) ? s.delete(label) : s.add(label); return s; });

  const clearAllFilters = () => { setActiveLevels(new Set()); setActiveDurations(new Set()); setActiveSubTechs(new Set()); };

  const filteredCourses = useMemo(() => {
    let arr = basePool.filter(c => {
      if (activeSubTechs.size > 0) {
        const matched = RELEVANT_TECHNOLOGIES
          .filter(t => activeSubTechs.has(t.label))
          .some(t => t.prefixes.some(p => c.code.startsWith(p)));
    ...
    return arr;
  }, [basePool, activeLevels, activeDurations, activeSubTechs, certSearch, certSort]);

  useEffect(() => { setCurrentPage(1); }, [activeTechName, activeLevels, activeDurations, activeSubTechs, certSearch, certSort]);

  const totalPages  = Math.max(1, Math.ceil(filteredCourses.length / ITEMS_PER_PAGE));
  const pagedCourses = filteredCourses.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
  const totalActiveFilters = activeLevels.size + activeDurations.size + activeSubTechs.size;
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const type = ((e as CustomEvent).detail?.type) as ("individual" | "enterprise" | undefined);
      setReqTab(type ?? "individual");
          ...
          <div className="certs-header-row">
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ color: "#0694D1", fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", lineHeight: 1 }}>MICROSOFT TRAINING</span>
              ...
              <p style={{ fontSize: 14, color: "#6b8499", margin: 0, maxWidth: 580, lineHeight: 1.6 }}>
                Build apps, automate workflows, analyze data, and create chatbots with Microsoft&apos;s low-code platform suite.
              </p>
            </div>

            {/* Mode toggle with shimmer border */}
            <div className="tab-shimmer-border cert-mode-toggle-wrap">
              <div style={{ display: "flex", alignItems: "stretch", background: "#fff", borderRadius: 16, padding: 5, gap: 4, width: "100%", boxSizing: "border-box" }}>
                {MODE_BTNS.map((btn, idx) => (
                  <button
                    key={btn.key}
                    onClick={() => setViewMode(btn.key)}
                    style={{
                      position: "relative", border: "none",
                      background: viewMode === btn.key ? "linear-gradient(135deg,#0694D1 0%,#046fa3 100%)" : "transparent",
                      padding: "11px 20px", borderRadius: 11, cursor: "pointer",
                      display: "inline-flex", alignItems: "center", gap: 9,
                      boxShadow: viewMode === btn.key ? "0 6px 20px rgba(6,148,209,0.5)" : "none",
                      transition: "background 0.22s,box-shadow 0.22s", fontFamily: "inherit",
                    }}
                  >
                    {idx === 0 && (
                      <div style={{ position: "absolute", right: -2, top: "20%", bottom: "20%", width: 1, background: "rgba(6,148,209,0.15)", pointerEvents: "none" }} />
                    )}
                    <span style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 8, background: viewMode === btn.key ? "rgba(255,255,255,0.18)" : "rgba(6,148,209,0.08)", flexShrink: 0 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={viewMode === btn.key ? "#fff" : "#0694D1"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{btn.icon}</svg>
                    </span>
                    <span style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 1 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: viewMode === btn.key ? "#fff" : "#6b8499", lineHeight: 1 }}>{btn.label}</span>
                      <span className="tab-sub-label" style={{ fontSize: 10, fontWeight: 500, lineHeight: 1, color: viewMode === btn.key ? "rgba(255,255,255,0.8)" : "#6b8499" }}>{btn.sub}</span>
                    </span>
                  </button>
                ))}
              </div>
        ...
        <div className="animate-on-scroll certs-layout" style={{ transitionDelay: "0.06s" }}>
          ...
          <div className="cert-sidebar">
            ...
            <div className="cert-tech-scroll">
              ...
              {filteredTechList.map(t => (
                <button
                  ...
                  onClick={() => { setActiveTechName(activeTechName === t.name ? "" : t.name); setActiveSubTechs(new Set()); }}
                >
                  <span className="csi-icon">{techLogos[t.name]}</span>
            ...
            <div className="cert-sidebar-filters">
              ...
              {/* Relevant Technology — always visible, 6 visible + scroll */}
              <div className="cert-sidebar-label">Relevant Technology</div>
              <div className="cert-filter-list" style={{ maxHeight: 190, overflowY: "auto", paddingRight: 2, scrollbarWidth: "thin", scrollbarColor: "rgba(6,148,209,0.25) transparent" }}>
                {RELEVANT_TECHNOLOGIES.map(tech => (
                  <button key={tech.label} className={`cert-filter-item${activeSubTechs.has(tech.label) ? " active" : ""}`} onClick={() => toggleSubTech(tech.label)}>
                    <span className="cert-filter-checkbox">
                      {activeSubTechs.has(tech.label) && (
                    ...
                    <span className="cert-filter-label">{tech.label}</span>
                  </button>
                ))}
              </div>
          ...
          <div className="cert-right">
            ...
            <div className="cert-tech-strip">
              {activeTech ? (
                <span style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(6,148,209,0.08)", borderRadius: 10, flexShrink: 0 }}>
                  {techLogos[activeTech.name]}
                </span>
              ) : (
                <span style={{ width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(6,148,209,0.08)", borderRadius: 10, flexShrink: 0 }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0694D1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
              ...
              <button className="cert-strip-req-btn" onClick={() => setReqInfoOpen(true)} style={{ flexShrink: 0, padding: "8px 18px", background: "linear-gradient(135deg,#0694D1,#046fa3)", color: "#fff", borderRadius: 8, fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 4px 14px rgba(6,148,209,0.25)", whiteSpace: "nowrap", fontFamily: "inherit" }}>
                Request More Info →
              </button>
            </div>
                  ...
                  <div key={`${course.code}-${i}`} className="cert-card">
                    ...
                    <div className="cert-footer">
                      ...
                      <div className="cert-actions">
                        <button className="cert-btn-brochure" onClick={() => { setSylCourse(course.title); setSyllabusOpen(true); }}>
                        ...
                        <a href="#contact" className="cert-btn-details">View Course</a>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <>
          ...
          <div className="tbs-sheet">
            ...
            <div className="tbs-list">
              {/* All Courses */}
              <button className={`tbs-item${activeTechName === "" ? " active" : ""}`} onClick={() => { setActiveTechName(""); setActiveSubTechs(new Set()); setTechSheetOpen(false); }}>
              ...
              {/* Tech list */}
              {technologies.map(t => (
                <button key={t.name} className={`tbs-item${activeTechName === t.name ? " active" : ""}`} onClick={() => { setActiveTechName(t.name); setActiveSubTechs(new Set()); setTechSheetOpen(false); }}>
            ...
            {sylSubmitted ? (
              ...
              <>
                ...
                <form onSubmit={e => { e.preventDefault(); if (!sylCountry) return; setSylSubmitted(true); }} style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  ...
                  <button type="submit" onClick={e => { if (!sylCountry) { e.preventDefault(); sylCountryRef.current?.querySelector("button")?.focus(); } }} style={{ width:"100%", padding:13, borderRadius:10, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#0694D1,#0577ab)", color:"#fff", fontSize:14, fontWeight:700, fontFamily:"inherit", letterSpacing:0.2, boxShadow:"0 4px 18px rgba(6,148,209,0.4)", marginTop:2, transition:"filter 0.18s" }}
              ...
              <>
                ...
                <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)", borderRadius: 10, padding: 4, gap: 4, marginBottom: 20 }}>
                  {(["individual", "enterprise"] as const).map(t => (
                    <button key={t} type="button" onClick={() => setReqTab(t)} style={{
                      ...
                      background: reqTab === t ? "#0694D1" : "transparent",
                      color: reqTab === t ? "#fff" : "rgba(255,255,255,0.45)",
                ...
                <form onSubmit={e => { e.preventDefault(); if (!reqCaptcha) return; setReqSubmitted(true); }} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div className="req-grid-2">
                    ...
                    <div>
                      <label className="req-label">{reqTab === "enterprise" ? "Business Email" : "Email"} <span style={{ color: "#ef4444" }}>*</span></label>
                      <input className="syl-input" type="email" placeholder={reqTab === "enterprise" ? "john@example.com" : "john@example.com"} required value={reqForm.email} onChange={e => setReqForm(f => ({ ...f, email: e.target.value }))} />
                    </div>
                    <div>
                    ...
                    <div>
                      {reqTab === "enterprise" ? (
                        ...
                        <>
                          <label className="req-label">Select Course Name</label>
                          <select className="syl-input req-select" value={reqForm.course} onChange={e => setReqForm(f => ({ ...f, course: e.target.value }))}>
                            <option value="">Select Course Name</option>
                            {technologies.flatMap(t => t.courses).map(c => (
                              <option key={c.code} value={c.title} style={{ background: "#0d2d47", color: "#c8d8e8" }}>{c.title}</option>
                            ))}
                          </select>
                        </>
                      )}
