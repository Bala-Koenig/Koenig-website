...
export default function ContactModal() {
  ...
  const [form, setForm] = useState({ fullName: "", email: "", phone: "", course: "", trainees: "", hearAbout: "", message: "" });

  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent).detail as { type?: "individual" | "enterprise" };
      const t = detail?.type ?? "individual";
      setInitType(t);
      setTab(t);
      setSubmitted(false);
      setRobotChecked(false);
      setForm({ fullName: "", email: "", phone: "", course: "", trainees: "", hearAbout: "", message: "" });
      setOpen(true);
    };
        ...
        <div className="p-6">
          {submitted ? (
            <div className="py-12 text-center">
              ...
              <button onClick={() => setOpen(false)} className="mt-6 rounded-xl bg-[#0694D1] px-6 py-2.5 text-sm font-semibold text-white hover:opacity-90 transition">
                Close
              </button>
            </div>
          ) : (
            <form onSubmit={e => { e.preventDefault(); setSubmitted(true); }}>
              ...
              <div className="mb-5 text-center">
                ...
                <p className="mt-1.5 text-sm text-white/50">Live Online Classroom Training with Koenig Solutions</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input type="text" required placeholder="John" value={form.fullName} onChange={e => set("fullName", e.target.value)} className={inputCls} style={inputStyle} />
                </div>
                <div>
                  <label className={labelCls}>Email <span className="text-red-400">*</span></label>
                  <input type="email" required placeholder="john@example.com" value={form.email} onChange={e => set("email", e.target.value)} className={inputCls} style={inputStyle} />
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select value={form.course} onChange={e => set("course", e.target.value)}
                        ...
                        style={{ ...inputStyle, color: form.course ? "#fff" : "rgba(255,255,255,0.3)" }}>
                        <option value="" style={{ background: "#06111E" }}>Select Course Name</option>
                        {courses.filter(Boolean).map(c => <option key={c} value={c} style={{ background: "#06111E", color: "#fff" }}>{c}</option>)}
                      </select>
              ...
              <div className="mt-3">
                ...
                <div className="relative">
                  <select value={form.hearAbout} onChange={e => set("hearAbout", e.target.value)}
                    ...
                    <option value="" style={{ background: "#06111E" }}>Select Option</option>
                    {hearOptions.filter(Boolean).map(o => <option key={o} value={o} style={{ background: "#06111E", color: "#fff" }}>{o}</option>)}
                  </select>
                  <svg className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2" style={{ color: "rgba(255,255,255,0.4)" }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6"/></svg>
                </div>
              </div>
