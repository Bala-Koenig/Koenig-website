...
/* ─── Mobile: card-by-card slider per vendor ─────────────────── */

function CardSlider({ vendor }: { vendor: typeof VENDOR_SECTIONS[0] }) {
  const [cardIdx, setCardIdx] = useState(0)
  const touchStartRef = useRef<number | null>(null)

  const prev = () => setCardIdx(i => (i - 1 + vendor.awards.length) % vendor.awards.length)
  const next = () => setCardIdx(i => (i + 1) % vendor.awards.length)
  ...
  const award = vendor.awards[cardIdx]
  ...
  return (
    <div>
      ...
      {/* Dots — only if more than 1 card */}
      {vendor.awards.length > 1 && (
        <div className="flex justify-center gap-1.5 mt-3">
          {vendor.awards.map((_, i) => (
            <button key={i} onClick={() => setCardIdx(i)} style={{
              width: i === cardIdx ? 22 : 7, height: 7,
              borderRadius: i === cardIdx ? 4 : '50%',
              background: i === cardIdx ? '#0694D1' : 'rgba(6,148,209,0.25)',
    ...
    return (
      <div className="flex flex-col gap-8">
        {VENDOR_SECTIONS.map(vendor => (
          <CardSlider key={vendor.name} vendor={vendor} />
        ))}
      </div>
    )
  }
